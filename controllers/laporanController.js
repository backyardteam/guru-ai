...
async function generatePDFSiswa(req, res) {
  ...
  const kelompok = await db.get(
    "SELECT * FROM kelompok_siswa WHERE siswa_id = ?",
    req.params.id,
  );
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename=laporan_${siswa.nama}.pdf`,
  );
  doc.pipe(res);
  ...
  if (kelompok) {
    ...
    doc.text(`Kelompok: ${kelompok.kelompok}`);
  }
  doc.end();
}
...
async function generatePDFKelas(req, res) {
  ...
  const hasil = await db.all(
    `SELECT s.nama, k.nilai, k.kelompok FROM kelompok_siswa k JOIN siswa s ON k.siswa_id = s.id`,
  );
  ...
  res.setHeader(
    "Content-Disposition",
    "attachment; filename=laporan_kelas.pdf",
  );
  doc.pipe(res);
  ...
  hasil.forEach((h) => {
    doc.text(
      `${h.nama} - Nilai: ${h.nilai || "-"} - Kelompok: ${h.kelompok || "-"}`,
    );
  });
  doc.end();
}

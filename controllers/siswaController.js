...
async function createSiswa(req, res) {
    ...
    try {
        await db.run(
            ...
            nis, nama, kelas || '3', jenis_kelamin, catatan_guru
        );
...
async function updateSiswa(req, res) {
    ...
    await db.run(
        ...
        nis, nama, kelas, jenis_kelamin, catatan_guru, req.params.id
    );
...
async function importCSV(req, res) {
    ...
    for (const row of data) {
        await db.run(
            ...
            row.nis, row.nama, row.kelas || '3', row.jenis_kelamin
        );
    }
    res.json({ success: true });
}
...
module.exports = { getAllSiswa, getSiswaById, createSiswa, updateSiswa, deleteSiswa, importCSV, exportExcel };